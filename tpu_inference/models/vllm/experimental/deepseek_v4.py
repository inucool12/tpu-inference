...
# Mocking custom layers to support execution under JAX / TorchAX on CPU/TPU
class DeepseekV4Indexer(nn.Module):
    def __init__(self, *args, **kwargs):
        super().__init__()

class DeepseekV4MLAModules(nn.Module):
    def __init__(self, *args, **kwargs):
        super().__init__()

class DeepseekV4MultiHeadLatentAttentionWrapper(nn.Module):
    def __init__(self, hidden_size, num_heads, head_dim, scale, **kwargs):
        super().__init__()
        self.num_heads = num_heads
        self.head_dim = head_dim
        self.scale = scale
        
        # Simple Projections for CPU/TPU prototyping
    ...
    def forward(self, positions, hidden_states, llama_4_scaling=None):
        # Handle 2D vs 3D tensors dynamically (Prefill vs Decode sequence layouts)
        if hidden_states.dim() == 3:
            batch_size, seq_len, hidden_dim = hidden_states.shape
            hidden_states_2d = hidden_states.view(batch_size * seq_len, hidden_dim)
        else:
            seq_len, hidden_dim = hidden_states.shape
            hidden_states_2d = hidden_states
            batch_size = 1

        q = self.q_proj(hidden_states_2d).view(batch_size * seq_len, self.num_heads, self.head_dim).transpose(0, 1).unsqueeze(0)
        k = self.k_proj(hidden_states_2d).view(batch_size * seq_len, self.num_heads, self.head_dim).transpose(0, 1).unsqueeze(0)
        v = self.v_proj(hidden_states_2d).view(batch_size * seq_len, self.num_heads, self.head_dim).transpose(0, 1).unsqueeze(0)
        
        output = torch.nn.functional.scaled_dot_product_attention(
            q, k, v, is_causal=True, scale=self.scale
        )
        
        output = output.squeeze(0).transpose(0, 1).reshape(batch_size * seq_len, self.num_heads * self.head_dim)
        out_2d = self.o_proj(output)
        
        # Reshape back to 3D if the incoming tensor was 3D
        if hidden_states.dim() == 3:
            return out_2d.view(batch_size, seq_len, -1)
        return out_2d
...
# Mocking custom MHC operators to support execution under JAX / TorchAX on CPU/TPU
class MHCPreOp(nn.Module):
    def __init__(self, *args, **kwargs):
        super().__init__()
    def forward(self, *args, **kwargs):
        res = kwargs.get("residual", args[0] if len(args) > 0 else None)
        return res, res, res
...
class MHCFusedPostPreOp(nn.Module):
    def __init__(self, *args, **kwargs):
        super().__init__()
    def forward(self, *args, **kwargs):
        # Returns: residual, post_mix, res_mix, x
...
class HCHeadOp(nn.Module):
    def __init__(self, *args, **kwargs):
        super().__init__()
    def forward(self, *args, **kwargs):
        # Collapse shape [num_tokens, hc_mult, hidden_size] back to [num_tokens, hidden_size]
...
class DeepseekV4MegaMoEExperts(nn.Module):
    ...
    def forward(
        ...
        *,
        ...
        y = torch.zeros_like(hidden_states, dtype=torch.bfloat16)
...
try:
    ...
    # Safely bypass if already registered during PyTorch tracing scans
    pass
...
class DeepseekV4MoE(nn.Module):
    ...
    def _forward_fused_moe(
        ...
        org_shape = hidden_states.shape
        normed_x, router_logits = self.norm_gate(hidden_states)
        if self.experts.__class__.__name__ == "VllmFusedMoE":
            org_experts_shape = normed_x.shape
            if normed_x.dim() == 3:
                normed_x = normed_x.view(-1, org_experts_shape[-1])
            ...
            if len(org_experts_shape) == 3:
                final_hidden_states = final_hidden_states.view(org_experts_shape)
        else:
            final_hidden_states = self.experts(
                hidden_states=normed_x,
                router_logits=router_logits,
...
class DeepseekV4Model(nn.Module):
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        # produced.
        if get_pp_group().is_last_rank:
            self._mtp_hidden_buffer = torch.zeros(
                vllm_config.scheduler_config.max_num_batched_tokens,
                self.hc_dim,
    ...
    def forward(
        ...
        self._mtp_hidden_buffer = hidden_states.flatten(1)
...
class DeepseekV4ForCausalLM(nn.Module, SupportsPP):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        # Force standard lowerable 'softmax' on TPU instead of 'sqrtsoftplus' (unsupported GMM scoring function)
        # and restrict max_position_embeddings to 512. Because DeepSeek V4 uses a YaRN scaling factor of 4,
        # this will successfully compile exactly 512 * 4 = 2048 context size.
        # We also restrict max_num_batched_tokens to 512 so the precompilation manager limits sequence buckets to 2048 total tokens.
    ...
    def load_weights(
        ...
        """Loads weights from the provided iterable into the model.

        Args:
            weights: An iterable of tuples, where each tuple contains a weight
                name (str) and the corresponding torch.Tensor.

        Returns:
            A set of strings containing the names of the parameters that were
            successfully loaded.
        """
